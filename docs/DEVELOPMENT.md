...
구체적인 절차는 **[배포 가이드](DEPLOY.md)** 에 따로 정리했다.

### 주의

- Azure SQL 무료 오퍼는 **월 10만 vCore-초 = 약 14시간**이다.
  **자동 일시중지를 끄면 반나절 만에 한 달치를 태우고 그달 내내 DB가 멈춘다.**
  일시중지는 켜두고, 지원 기간에만 cron으로 `/health/db` 를 찔러 깨워둔다.
